...
func run(c *exec.Cmd, timeout time.Duration) error {
	if timeout <= 0 {
		...
		case <-time.After(timeout):
			err = c.Process.Kill()
			if err != nil {
				sdlog.WithError(err).WithField("pid", c.Process.Pid).Debug("kill timeout process error")
			}
			return ErrTimeout
		case err := <-done:
...
func combinedOutput(c *exec.Cmd, timeout time.Duration) ([]byte, error) {
	if c.Stdout != nil {
		return nil, sderr.New("exec: Stdout already set")
	}
	if c.Stderr != nil {
		return nil, sderr.New("exec: Stderr already set")
	}
	var b bytes.Buffer
	c.Stdout = &b
	c.Stderr = &b
