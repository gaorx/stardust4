...
var (
	ErrIllegalDriver = sderr.New("illegal driver")
)
...
func Dial(addr Address, config *gorm.Config) (*gorm.DB, error) {
	...
	case "mysql":
		...
		if err != nil {
			return nil, sderr.Wrap(err, "sdgorm open mysql error")
		}
		return db, nil
	case "postgres":
		...
		if err != nil {
			return nil, sderr.Wrap(err, "sdgorm open postgres error")
		}
		return db, nil
	case "sqlite":
		db, err := gorm.Open(sqlite.Open(addr.DSN), config)
		if err != nil {
			return nil, sderr.Wrap(err, "sdgorm open sqlite error")
		}
		return db, nil
	default:
